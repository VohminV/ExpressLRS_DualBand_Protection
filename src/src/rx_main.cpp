...
static uint8_t minLqForChaos()
{
    ...
    const uint32_t numfhss = FHSSgetChannelCount(SX12XX_Radio_1);
...
void SetRFLinkRate(uint8_t index, bool bindMode) // Set speed of RF link
{
    ...
    Radio.Config(ModParams->bw, ModParams->sf, ModParams->cr, FHSSgetInitialFreq(SX12XX_Radio_1),
                 ModParams->PreambleLen, invertIQ, ModParams->PayloadLength, 0,  SX12XX_Radio_1
    ...
    Radio.Config(ModParams->bw, ModParams->sf, ModParams->cr, FHSSgetInitialFreq(SX12XX_Radio_2),
                 ModParams->PreambleLen, invertIQ, ModParams->PayloadLength, 0,  SX12XX_Radio_2
#if defined(RADIO_SX128X)
                 , uidMacSeedGet(), OtaCrcInitializer, (ModParams->radio_type == RADIO_TYPE_SX128x_FLRC)
#endif
                 );
    ...
    cycleInterval = ((uint32_t)11U * FHSSgetChannelCount(SX12XX_Radio_1) * ModParams->FHSShopInterval * interval) / (10U * 1000U);
...
bool ICACHE_RAM_ATTR HandleFHSS()
{
	...
	Radio.SetFrequencyReg(FHSSgetNextFreq(SX12XX_Radio_1), SX12XX_Radio_1);
	Radio.SetFrequencyReg(FHSSgetNextFreq(SX12XX_Radio_2), SX12XX_Radio_2);
...
bool ICACHE_RAM_ATTR HandleSendTelemetryResponse()
{
    ...
    Radio.TXnb((uint8_t*)&otaPkt, ExpressLRS_currAirRate_Modparams->PayloadLength, SX12XX_Radio_1);
    Radio.TXnb((uint8_t*)&otaPkt, ExpressLRS_currAirRate_Modparams->PayloadLength, SX12XX_Radio_2);
...
{
    Radio.currFreq = FHSSgetInitialFreq(SX12XX_Radio_1);
    Radio.currFreq2 = FHSSgetInitialFreq(SX12XX_Radio_2);
#if defined(RADIO_SX127X)
...
static void EnterBindingMode()
{
    ...
    Radio.SetFrequencyReg(FHSSgetInitialFreq(SX12XX_Radio_2),SX12XX_Radio_2);
