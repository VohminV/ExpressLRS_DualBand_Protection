...
volatile bool busyTransmitting1;
volatile bool busyTransmitting2;
...
bool ICACHE_RAM_ATTR ProcessTLMpacket(SX12xxDriverCommon::rx_status const status, SX12XX_Radio_Number radioNumber)
{
...
void SetRFLinkRate(uint8_t index) // Set speed of RF link
{
  ...
  Radio.Config(ModParams->bw, ModParams->sf, ModParams->cr, FHSSgetInitialFreq(SX12XX_Radio_1),
               ModParams->PreambleLen, invertIQ, ModParams->PayloadLength, ModParams->interval, SX12XX_Radio_1
#if defined(RADIO_SX128X)
...
void ICACHE_RAM_ATTR HandleFHSS(SX12XX_Radio_Number radioNumber)
{
  ...
  {
	Radio.SetFrequencyReg(FHSSgetNextFreq(radioNumber), radioNumber);
  }
}

void ICACHE_RAM_ATTR HandlePrepareForTLM(SX12XX_Radio_Number_t radioNumber)
{
...
void ICACHE_RAM_ATTR SendRCdataToRF()
{
  ...
  else if ((!skipSync) && ((syncSlot / 2) <= NonceFHSSresult) && (now - SyncPacketLastSent > SyncInterval) && FHSSonSyncChannel(SX12XX_Radio_1))
  {
    otaPkt.std.type = PACKET_TYPE_SYNC;
    GenerateSyncPacketData(OtaIsFullRes ? &otaPkt.full.sync.sync : &otaPkt.std.sync);
...
#else
    CRSF::LinkStatistics.downlink_Link_quality = LQCalc1.getLQ();
#endif
    LQCalc1.inc();
    return;
  }
  else if (TelemetryRcvPhase == ttrpExpectingTelem && !LQCalc1.currentIsSet())
  {
...
static void CheckConfigChangePending()
{
  if (config.IsModified() || ModelUpdatePending)
  {
    ...
    while (busyTransmitting1); // wait until no longer transmitting
#else
...
bool ICACHE_RAM_ATTR RXdoneISR(SX12xxDriverCommon::rx_status const status, SX12XX_Radio_Number radioNumber)
{
  if (radioNumber == SX12XX_Radio_1)
  {
	if (LQCalc1.currentIsSet())
	{
		return false; // Already received tlm, do not run ProcessTLMpacket() again.
	}

	bool packetSuccessful = ProcessTLMpacket(status, radioNumber);
	busyTransmitting1 = false;
	return packetSuccessful;
  }
  else if (radioNumber == SX12XX_Radio_1)
    {
        if (LQCalc2.currentIsSet())
        {
            return false;
        }
        ...
        bool packetSuccessful = ProcessTLMpacket(status, radioNumber);
            ...
            if (packetSuccessful)
            {
                SetClearChannelAssessmentTime(radioNumber);
            }
        #endif
        busyTransmitting2 = false;
        return packetSuccessful;
    }
...
void ICACHE_RAM_ATTR TXdoneISR(SX12XX_Radio_Number_t radioNumber)
{
  if (radioNumber == SX12XX_Radio_1)
    {
        if (!busyTransmitting1)
        {
            return; // Уже завершена передача
        }

        if (connectionState != awaitingModelId)
        {
            HandleFHSS(radioNumber);
            HandlePrepareForTLM(radioNumber);
            #if defined(Regulatory_Domain_EU_CE_2400)
                if (TelemetryRcvPhase1 != ttrpPreReceiveGap)
                {
                    SetClearChannelAssessmentTime(radioNumber);
                }
            #endif
    ...
    else if (radioNumber == SX12XX_Radio_2)
    {
        ...
        if (connectionState != awaitingModelId)
        {
            ...
            HandlePrepareForTLM(radioNumber);
            #if defined(Regulatory_Domain_EU_CE_2400)
                if (TelemetryRcvPhase2 != ttrpPreReceiveGap)
                {
                    SetClearChannelAssessmentTime(radioNumber);
                }
            #endif
...
bool ICACHE_RAM_ATTR RXdoneCallbackWrapper(SX12xxDriverCommon::rx_status const status) {
    RXdoneISR(status, SX12XX_Radio_1);
}


void ICACHE_RAM_ATTR TXdoneCallbackWrapper() {
    TXdoneISR(SX12XX_Radio_1);
}
...
static void EnterBindingMode()
{
  ...
  Radio.SetFrequencyReg(FHSSgetInitialFreq(SX12XX_Radio_1), SX12XX_Radio_1);
...
void setup()
{
  if (setupHardwareFromOptions())
  {
    ...
    FHSSrandomiseFHSSsequence(uidMacSeedGet(),SX12XX_Radio_1);
    //FHSSrandomiseFHSSsequence(uidMacSeedGet(),SX12XX_Radio_2);

    //Radio.RXdoneCallback = &RXdoneISR(SX12XX_Radio_1);
    //Radio.TXdoneCallback = &TXdoneISR(SX12XX_Radio_1);
	Radio.RXdoneCallback = RXdoneCallbackWrapper;
	Radio.TXdoneCallback = TXdoneCallbackWrapper;
    ...
    Radio.currFreq = FHSSgetInitialFreq(SX12XX_Radio_1); //set frequency first or an error will occur!!!
    //Radio.currFreq2 = FHSSgetInitialFreq(SX12XX_Radio_2); //set frequency first or an error will occur!!!
